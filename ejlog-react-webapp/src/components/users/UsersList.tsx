...
import { useUsers, useUpdateUser, useChangePassword } from '@/hooks/useUsers';
...
export const UsersList: FC<UsersListProps> = ({ currentUserId, isAdmin }) => {
  ...
  const canToggleLock = (user: User) =>
    isAdmin && user.utente.toLowerCase() !== 'superuser';
  ...
  const handleResetPassword = (user: User) => {
    if (!window.confirm(`Reimpostare la password di ${user.utente} a 'promag'?`)) {
      return;
    }
              ...
              <DropdownMenuContent align="end">
                ...
                {canToggleLock(user) && (
                  <DropdownMenuItem onClick={() => handleToggleLock(user)}>
                    <UserCog className="h-4 w-4 mr-2" />
                    {user.lockPpcLogin ? 'Sblocca PPC' : 'Blocca PPC'}
                  </DropdownMenuItem>
                )}
