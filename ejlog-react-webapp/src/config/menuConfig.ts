...
export interface MenuItem {
  ...
  showOnlyInTouchMode?: boolean; // Mostra solo quando touch mode ̬ attivo
  hideInTouchMode?: boolean; // Nascondi quando touch mode ̬ attivo
  // Moduli opzionali
...
// ============================================================================
// MENU CONFIGURATION
const ppcModuleIconMap: Record<string, LucideIcon> = {
  menu: Boxes,
  layout: Layers,
  login: Users,
...
const ppcModuleDefaultRoutes: Record<string, string> = {
  menu: '/ppc/menu/main-menu',
  login: '/ppc/login/login',
  errors: '/ppc/errors/error-details',
  operator: '/ppc/operator/operator-menu',
  installation: '/ppc/menu/installation-menu',
  layout: '/ppc/layout/layout',
};

const ppcModuleOrder = ['menu', 'login', 'errors', 'operator', 'installation', 'layout'];

const ppcModuleMenuItems: MenuItem[] = ppcModules
  .filter((module) => module.projectSlug === 'modules')
  .sort((a, b) => {
    const indexA = ppcModuleOrder.indexOf(a.slug);
    const indexB = ppcModuleOrder.indexOf(b.slug);
  ...
  .map((module) => ({
    id: `ppc-${module.slug}`,
    icon: ppcModuleIconMap[module.slug] ?? Monitor,
    label: module.label,
    path: ppcModuleDefaultRoutes[module.slug] ?? `/ppc/${module.slug}`,
  ...
  return menu
    ...
    .map((item) => {
      if (item.children && item.children.length > 0) {
        ...
        // Se il menu ha figli ma nessuno ̬ accessibile, nasconde il parent
