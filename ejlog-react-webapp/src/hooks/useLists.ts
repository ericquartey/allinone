...
export const useLists = (filters?: ListFilters) => {
  ...
  const fetchLists = useCallback(async () => {
    ...
    try {
      ...
      const exported = response.exported || response.exportedItems || [];
      if (response.result === 'OK') {
        console.log(`✅ useLists: Got ${exported.length} lists from backend`);

        // Transform ServiceList[] to ListExport[] format expected by UI
        const transformedLists: ListExport[] = exported.map((list: ServiceList) => ({
          listHeader: {
            ...list.listHeader,
        ...
        setTotalCount(response.recordNumber ?? exported.length);
      } else {
