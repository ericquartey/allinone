...
class APIClient {
  ...
  private async fetch<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
    const { primary, fallback } = getLegacyBaseUrlCandidates();
    const bases = [primary, fallback].filter(Boolean) as string[];

    if (bases.length === 0) {
      throw new Error('Legacy API disabilitata');
    }

    let lastError: Error | null = null;
    for (const baseUrl of bases) {
      try {
        return await this.fetchWithBase<T>(baseUrl, endpoint, options);
      } catch (error: any) {
    ...
    throw new Error('Legacy API non raggiungibile');
  }
