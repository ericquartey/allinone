...
const StockPageEnhanced: React.FC = () => {
  ...
  const handleExport = (format: 'CSV' | 'EXCEL' | 'PDF') => {
    if (!stock || stock.length === 0) {
      toast.error('Nessuna giacenza da esportare');
      return;
    }
    if (!hasSearched) {
      toast.error('Esegui una ricerca prima di esportare');
      return;
    }
    const columns = [
      ...
      { header: 'Seriale', accessor: 'serialNumber' },
      { header: 'Scadenza', accessor: 'expiryDate' },
      { header: 'Quantita', accessor: 'qty' },
      { header: 'Magazzino', accessor: 'warehouseId' },
    ];
    if (groupByTray) {
      columns.push({ header: 'UDC', accessor: 'LU' });
    }
    exportData(format, {
      filename: `giacenze_${new Date().toISOString().split('T')[0]}`,
