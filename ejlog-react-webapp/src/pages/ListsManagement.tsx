...
export default function ListsManagementComplete() {
  ...
  const getSelectedListDetail = useCallback(async () => {
    if (!selectedList) return null;
    const listNumber = selectedList.listHeader.listNumber;
    return await fetchListDetail(listNumber);
  }, [selectedList, fetchListDetail]);

  const handleExportSelectedRows = useCallback(async (format: 'CSV' | 'EXCEL' | 'PDF') => {
    if (!selectedList) {
      toast.error('Seleziona una lista prima di esportare');
      return;
    }
    if (selectedRows.length === 0) {
      toast.error('Seleziona una o piu righe');
      return;
    }
    const listNumber = selectedList.listHeader?.listNumber || 'lista';
    exportData(format, {
      filename: `righe_lista_${listNumber}`,
      title: `Righe Lista ${listNumber}`,
      columns: [
        { header: 'Riga', accessor: 'rowNumber' },
        { header: 'Articolo', accessor: 'item' },
        { header: 'Descrizione', accessor: 'lineDescription' },
        { header: 'Qta Richiesta', accessor: 'requestedQty' },
        { header: 'Qta Evasa', accessor: 'processedQty' },
        { header: 'Lotto', accessor: 'lot' },
        { header: 'Matricola', accessor: 'serialNumber' },
        { header: 'Scadenza', accessor: 'expiryDate' },
      ],
      data: selectedRows,
    });
    toast.success(`Export righe ${format} avviato`);
  }, [selectedList, selectedRows]);

  const handlePrintDetailPdf = useCallback(async () => {
    if (!selectedList) {
      toast.error('Seleziona una lista prima di stampare');
      return;
    }
    const detail = await getSelectedListDetail();
    if (!detail) {
      toast.error('Dettaglio lista non disponibile');
      return;
    }
    ...
    const rowsHtml = rows.map((row: any) => `
      <tr>
        <td>${row.rowNumber || row.rowSequence || ''}</td>
        <td>${row.item || ''}</td>
        <td>${row.lineDescription || ''}</td>
        <td>${row.requestedQty ?? ''}</td>
        <td>${row.processedQty ?? ''}</td>
    ...
    `).join('');
    const html = `
      <h1>Dettaglio Lista ${header.listNumber}</h1>
      <div><strong>Descrizione:</strong> ${header.listDescription || '-'}</div>
      <div><strong>Tipo:</strong> ${header.listType}</div>
      <div><strong>Stato:</strong> ${header.listStatus}</div>
      <div><strong>Priorita:</strong> ${header.priority}</div>
      <table>
        <thead>
          <tr>
            <th>Riga</th>
    ...
    `;
    openPrintWindow(`Dettaglio ${header.listNumber}`, html);
  }, [selectedList, getSelectedListDetail]);

  const handleExportLists = useCallback((format: 'CSV' | 'EXCEL' | 'PDF') => {
    if (!filteredLists || filteredLists.length === 0) {
      toast.error('Nessuna lista da esportare');
      return;
    }
    const rows = filteredLists.map((listExport) => ({
      listNumber: listExport.listHeader?.listNumber,
      description: listExport.listHeader?.listDescription,
      type: listExport.listHeader?.listType,
      status: listExport.listHeader?.listStatus,
      priority: listExport.listHeader?.priority,
      createdAt: listExport.listHeader?.createdAt || '',
      completedAt: listExport.listHeader?.completedAt || '',
    }));
    exportData(format, {
      filename: `liste_${new Date().toISOString().split('T')[0]}`,
      title: 'Liste',
      columns: [
        { header: 'Numero Lista', accessor: 'listNumber' },
        { header: 'Descrizione', accessor: 'description' },
        { header: 'Tipo', accessor: 'type' },
        { header: 'Stato', accessor: 'status' },
        { header: 'Priorita', accessor: 'priority' },
        { header: 'Creata Il', accessor: 'createdAt' },
        { header: 'Terminata Il', accessor: 'completedAt' },
      ],
      data: rows,
    });
    toast.success(`Export ${format} avviato`);
  }, [filteredLists]);
    ...
    <div className="h-screen flex flex-col bg-ferretto-gray-50">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <ListsSidebar
          ...
          onExportCsv={() => handleExportLists('CSV')}
          onExportExcel={() => handleExportLists('EXCEL')}
          onExportPdf={() => handleExportLists('PDF')}
          onExportRowsCsv={() => handleExportSelectedRows('CSV')}
          onExportRowsExcel={() => handleExportSelectedRows('EXCEL')}
          onExportRowsPdf={() => handleExportSelectedRows('PDF')}
          onPrintDetailPdf={handlePrintDetailPdf}
