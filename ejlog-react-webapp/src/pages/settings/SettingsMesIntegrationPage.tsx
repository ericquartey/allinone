// ============================================================================
// EJLOG WMS - MES Integration Settings Page
// ============================================================================

import React, { useEffect, useState } from 'react';
import { ArrowPathIcon, CheckCircleIcon, ExclamationTriangleIcon } from '@heroicons/react/24/outline';
import { integrationsApi } from '../../services/integrationsApi';
import type { MesIntegrationConfig } from '../../types/integrations';

type TestState = 'idle' | 'testing' | 'success' | 'error';

export default function SettingsMesIntegrationPage() {
  const [draft, setDraft] = useState<MesIntegrationConfig | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setLoading(true);
        const response = await integrationsApi.getIntegration<MesIntegrationConfig>('mes');
        if (!mounted) return;
        setDraft(response.data.config);
        setLoadError(null);
      } catch (error: any) {
        if (!mounted) return;
        setLoadError(error?.message || 'Errore caricamento integrazione MES');
      } finally {
  ...
  const updateField = (field: keyof MesIntegrationConfig, value: any) => {
    setDraft((prev) => (prev ? { ...prev, [field]: value } : prev));
  };

  const updateNested = (section: keyof MesIntegrationConfig, field: string, value: any) => {
    setDraft((prev) => {
      if (!prev) return prev;
      return {
        ...prev,
  ...
  const handleSave = async () => {
    if (!draft) return;
    await integrationsApi.saveIntegration('mes', {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      const response = await integrationsApi.testIntegration('mes', {
        enabled: draft.enabled,
        config: draft,
      });
      if (response.success) {
      ...
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="max-w-5xl mx-auto">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            Caricamento configurazione MES...
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="max-w-5xl mx-auto text-sm text-gray-600">
          Configurazione MES non disponibile.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-5xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Integrazione MES</h1>
          <p className="text-gray-600">
            Collega il MES per ordini produzione, consumi e avanzamenti.
          </p>
        </div>
        ...
        <div className={sectionClass}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600">Abilita o disabilita lo scambio MES.</p>
            </div>
        ...
        <div className={sectionClass}>
          <h2 className="text-lg font-semibold text-gray-900">Endpoint MES</h2>
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {draft.mode === 'MQTT' && (
          <div className={sectionClass}>
            <h2 className="text-lg font-semibold text-gray-900">MQTT</h2>
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateNested('mqtt', 'brokerUrl', e.target.value)}
                  className={inputClass}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateNested('mqtt', 'username', e.target.value)}
                  className={inputClass}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateNested('mqtt', 'password', e.target.value)}
                  className={inputClass}
                />
              </div>
              <div className="md:col-span-2">
                ...
                <input
                  type="text"
                  value={draft.mqtt.topics}
                  onChange={(e) => updateNested('mqtt', 'topics', e.target.value)}
                  className={inputClass}
