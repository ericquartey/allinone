// ============================================================================
// EJLOG WMS - TMS Integration Settings Page
// ============================================================================

import React, { useEffect, useState } from 'react';
import { ArrowPathIcon, CheckCircleIcon, ExclamationTriangleIcon } from '@heroicons/react/24/outline';
import { integrationsApi } from '../../services/integrationsApi';
import type { TmsIntegrationConfig } from '../../types/integrations';

type TestState = 'idle' | 'testing' | 'success' | 'error';

export default function SettingsTmsIntegrationPage() {
  const [draft, setDraft] = useState<TmsIntegrationConfig | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setLoading(true);
        const response = await integrationsApi.getIntegration<TmsIntegrationConfig>('tms');
        if (!mounted) return;
        setDraft(response.data.config);
        setLoadError(null);
      } catch (error: any) {
        if (!mounted) return;
        setLoadError(error?.message || 'Errore caricamento integrazione TMS');
      } finally {
  ...
  const updateField = (field: keyof TmsIntegrationConfig, value: any) => {
    setDraft((prev) => (prev ? { ...prev, [field]: value } : prev));
  };

  const updateNested = (section: keyof TmsIntegrationConfig, field: string, value: any) => {
    setDraft((prev) => {
      if (!prev) return prev;
      return {
        ...prev,
  ...
  const handleSave = async () => {
    if (!draft) return;
    await integrationsApi.saveIntegration('tms', {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      const response = await integrationsApi.testIntegration('tms', {
        enabled: draft.enabled,
        config: draft,
      });
      if (response.success) {
      ...
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="max-w-5xl mx-auto">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            Caricamento configurazione TMS...
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="max-w-5xl mx-auto text-sm text-gray-600">
          Configurazione TMS non disponibile.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-5xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Integrazione TMS</h1>
          <p className="text-gray-600">
            Collega il Transport Management System per spedizioni e tracking.
          </p>
        </div>
        ...
        <div className={sectionClass}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600">Abilita o disabilita lo scambio TMS.</p>
            </div>
        ...
        <div className={sectionClass}>
          <h2 className="text-lg font-semibold text-gray-900">Endpoint TMS</h2>
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
