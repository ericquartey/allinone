...
export const useDrawers = (filters?: DrawerFilters) => {
  ...
  const callDrawer = useCallback(async (id: number, destinationId?: number): Promise<LoadingUnit | null> => {
    setLoading(true);
    setError(null);

    try {
      const drawer = await drawersApi.callLoadingUnit(id, destinationId);
      await fetchDrawers(); // Refresh the list
      return drawer;
    } catch (err: any) {
