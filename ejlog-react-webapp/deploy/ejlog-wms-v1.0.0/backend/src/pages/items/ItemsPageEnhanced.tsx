...
const ItemsPageEnhanced: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Transform response - BACKEND JAVA FORMAT (exported, not data)
        ...
        const transformedData = {
          ...
          total: response.data.recordNumber || response.data.pagination?.total || items.length,
          totalPages: Math.ceil((response.data.recordNumber || response.data.pagination?.total || items.length) / filters.pageSize),
        };
  ...
  const handleExport = (format: 'csv' | 'excel') => {
    ...
    // TODO: Implementare export reale chiamando API backend
  };
  ...
  const handleDeleteItem = () => {
    if (itemToDelete) {
      ...
      // TODO: Implementare delete chiamando API
