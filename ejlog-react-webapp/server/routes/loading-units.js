...
/**
 * POST /api/EjLogHostVertimag/api/loading-units/:id/call
 * Chiama una UDC in baia (crea missione in coda)
 */
router.post('/:id/call', async (req, res) => {
  try {
    const pool = await getPool();
    const { id } = req.params;
    const destinationIdRaw = req.query.destinationId;
    const destinationId = destinationIdRaw !== undefined ? Number(destinationIdRaw) : null;
    ...
    if (udcResult.recordset.length === 0) {
      return res.status(404).json({ error: 'UDC non trovata' });
    }

    const udc = udcResult.recordset[0];
    let resolvedDestinationId = Number.isFinite(destinationId) ? destinationId : null;
    ...
    if (!resolvedDestinationId) {
      try {
        const locationResult = await pool.request().query(`
          ...
          WHERE recordCancellato = 0
            AND (
              UPPER(codice) LIKE '%BAIA%' OR UPPER(codice) LIKE '%BAY%' OR
              UPPER(descrizione) LIKE '%BAIA%' OR UPPER(descrizione) LIKE '%BAY%'
            )
          ORDER BY id
        `);
        resolvedDestinationId = locationResult.recordset[0]?.id || null;
      } catch (error) {
        console.warn('[POST /loading-units/:id/call] Unable to resolve default bay location:', error?.message || error);
      }
    }

    if (!resolvedDestinationId) {
      return res.status(400).json({
        error: 'Destinazione non valida. Specifica destinationId per richiamare il cassetto.',
      });
    }
    ...
    if (existingMissionResult.recordset.length > 0) {
      return res.status(400).json({
        error: 'Esiste gia una missione attiva per questa UDC',
      });
    }

    const existingBufferResult = await pool.request()
      .input('idUdc', id)
      .query(`
        SELECT id
    ...
    if (existingBufferResult.recordset.length > 0) {
      return res.status(400).json({
        error: 'Esiste gia una missione in coda per questa UDC',
      });
    }
    ...
    const insertResult = await pool.request()
      ...
      .input('idLocazioneDeposito', resolvedDestinationId)
      .input('tipoMissione', 'PR')
      .input('causale', 'Richiamo cassetto')
      .query(`
    ...
    const idBuffer = insertResult.recordset[0]?.id || null;
    ...
    res.json({
      ...
      data: {
        ...
        idLocazionePrelievo: udc.idLocazione,
        idLocazioneDeposito: resolvedDestinationId,
        message: 'Richiamo cassetto inserito in coda',
      },
    });
  } catch (err) {
    console.error('[API] Error calling loading unit:', err);
    res.status(500).json({
      error: 'Errore nel richiamo UDC',
      message: err.message,
    });
  }
});
