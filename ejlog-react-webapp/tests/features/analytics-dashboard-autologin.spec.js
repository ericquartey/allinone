...
test.describe('Analytics Dashboard with Auto-Login', () => {
  test.beforeEach(async ({ page }) => {
    // Navigate to home page first to trigger auto-login
    await page.goto('http://localhost:3002/');

    // Wait for auto-login to complete by checking localStorage
    await page.waitForFunction(() => {
      const token = localStorage.getItem('token');
      const user = localStorage.getItem('user');
      return token === 'dev-token-superuser' && user !== null;
    }, { timeout: 5000 });
  ...
  test('should take screenshot of full dashboard', async ({ page }) => {
    ...
    await page.screenshot({
      path: 'test-results/analytics-dashboard-autologin-full.png',
      fullPage: true
    });

    console.log('Screenshot saved to test-results/analytics-dashboard-autologin-full.png');
  });

  test('should verify auto-login is working', async ({ page }) => {
    // Check that we're authenticated
    const token = await page.evaluate(() => localStorage.getItem('token'));
    const userStr = await page.evaluate(() => localStorage.getItem('user'));

    expect(token).toBe('dev-token-superuser');
    ...
    console.log('Auto-login verified - User:', user.username, 'Roles:', user.roles);
  });
});
