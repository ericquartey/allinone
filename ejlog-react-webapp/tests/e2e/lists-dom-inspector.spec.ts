/**
 * Lists DOM Structure Inspector
 *
 * This test inspects the actual DOM structure of the Lists page
 * to identify correct selectors for writing robust E2E tests.
 *
 * Run with: npx playwright test tests/e2e/lists-dom-inspector.spec.ts --headed
 */
...
test.describe('Lists DOM Inspector', () => {
  test('inspect lists page structure', async ({ page }) => {
    console.log('🔍 Inspecting Lists page DOM structure...\n');
    ...
    const screenshotPath = path.join(__dirname, '../../test-results/lists-dom-structure.png');
    ...
    // Print structure
    console.log('📋 LISTS PAGE STRUCTURE:\n');
    ...
    const jsonPath = path.join(__dirname, '../../test-results/lists-dom-structure.json');
    fs.writeFileSync(jsonPath, JSON.stringify(structure, null, 2));
    console.log(`\n💾 Structure saved to: ${jsonPath}`);

    // Save full HTML
    const html = await page.content();
    const htmlPath = path.join(__dirname, '../../test-results/lists-full.html');
    fs.writeFileSync(htmlPath, html);
    console.log(`📄 Full HTML saved to: ${htmlPath}`);

    console.log('\n✅ Lists DOM inspection complete!');
  });
});
